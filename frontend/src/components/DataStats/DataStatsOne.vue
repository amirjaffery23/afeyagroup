...
const cardItems = ref([
  {
    ...
    points: null, // Dynamically updated via WebSocket
    total: "Down Jones Indu...",
    growthRate: null, // Dynamically updated via WebSocket
  },
  {
    icon: `<svg
            class="fill-primary dark:fill-white"
  ...
  >
    ...
    <div class="mt-4 flex items-end justify-between">
      <div>
        ...
        <span class="text-sm font-medium">{{ item.points || "-" }}</span>
      </div>

      <span
        class="flex items-center gap-1 text-sm font-medium"
        :class="{
          'text-meta-3': item.growthRate !== null && item.growthRate > 0,
          'text-meta-5': item.growthRate !== null && item.growthRate < 0,
        }"
      >
        {{ item.growthRate || "-" }}%
        <svg
          v-if="item.growthRate !== null && item.growthRate > 0"
          class="fill-meta-3"
          width="10"
          height="11"
          viewBox="0 0 10 11"
